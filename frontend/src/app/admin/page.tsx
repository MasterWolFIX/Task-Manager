    ...
    return (
      ...
      `}>
                ...
                <nav className="flex-1 px-3 space-y-1">
                    <Link href="/admin" onClick={() => setSidebarOpen(false)} className="flex items-center px-4 py-2.5 bg-blue-600/10 text-blue-400 rounded-xl font-black uppercase text-[11px] tracking-widest border border-blue-600/20 shadow-md">Dashboard</Link>
                    <Link href="/admin/classes" onClick={() => setSidebarOpen(false)} className="flex items-center px-4 py-2.5 text-zinc-400 hover:text-white rounded-xl transition-all font-black uppercase text-[11px] tracking-widest">Klasy</Link>
                    <Link href="/admin/settings" onClick={() => setSidebarOpen(false)} className="flex items-center px-4 py-2.5 text-zinc-400 hover:text-white rounded-xl transition-all font-black uppercase text-[11px] tracking-widest">Ustawienia</Link>
                </nav>
                ...
                <div className="p-3 border-t border-white/5">
                    <button
                        ...
                        className="w-full text-[11px] font-black uppercase tracking-widest text-red-500 border border-red-600/20 hover:bg-red-600 hover:text-white px-3 py-2.5 rounded-lg transition-all"
                    >
                        LOGOUT
                    </button>
                </div>
            ...
            <section className="flex-1 flex flex-col overflow-hidden bg-[#050505] min-w-0">
                <header className="h-14 border-b border-white/5 flex items-center justify-between px-4 sm:px-8 bg-[#050505]/80 backdrop-blur-3xl z-20 shrink-0">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => router.push('/admin/tasks/new')} className="bg-white hover:bg-zinc-200 text-black px-4 sm:px-5 py-1.5 rounded-lg font-black uppercase text-[11px] tracking-widest">+ NOWE</button>
                    </div>
                </header>
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-3">
                        {[
                            ...
                            <div key={i} className="bg-[#0a0a0a] border border-white/5 p-3 sm:p-4 rounded-2xl relative group overflow-hidden">
                                <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-500 mb-2 group-hover:text-white transition-colors truncate">{s.label}</p>
                                        ...
                                        <div className="flex items-center gap-4 sm:gap-6 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                                                    <h3 className="text-[13px] font-bold text-white tracking-tight group-hover:text-blue-500 transition-colors uppercase leading-none truncate">{task.title}</h3>
                                                    <span className="text-[10px] font-black uppercase px-1.5 py-0.5 bg-zinc-900 text-zinc-500 rounded border border-white/5 shrink-0">{task.language}</span>
                                                ...
                                                <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest italic">{new Date(task.deadline).toLocaleString()}</p>
                                            </div>
                                        </div>

                                        <div className="flex items-center gap-2 shrink-0 pl-12 sm:pl-0">
                                            <button onClick={() => router.push(`/admin/tasks/${task.id}`)} className="h-8 px-3 sm:px-4 rounded-lg bg-blue-600 hover:bg-blue-500 text-white transition-all font-black uppercase text-[10px] tracking-widest shadow-lg shadow-blue-900/10">PANEL OCEN</button>
                                            <button onClick={() => router.push(`/admin/tasks/${task.id}/edit`)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-zinc-900 border border-white/5 hover:border-white/20 text-zinc-500 hover:text-white transition-all"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                            <button onClick={() => handleDeleteTask(task.id)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-600/5 hover:bg-red-600 hover:text-white text-red-500 border border-red-600/10 transition-all"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
