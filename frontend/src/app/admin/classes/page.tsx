                       ...
                       <div className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[24px] shadow-2xl space-y-4">
                            ...
                            {addMode === 'single' ? (
                                <form onSubmit={handleCreateSingle} className="space-y-2">
                                    <input required placeholder="IMIĘ I NAZWISKO" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} className="w-full bg-black border border-white/5 rounded-xl p-3 text-[9px] font-black text-white uppercase outline-none" />
                                    <input required type="email" placeholder="ADRES E-MAIL" value={newStudentEmail} onChange={e => setNewStudentEmail(e.target.value)} className="w-full bg-black border border-white/5 rounded-xl p-3 text-[9px] font-black text-white uppercase outline-none" />
                          ...
                          <div key={c.id} className="bg-[#0a0a0a] border border-white/5 rounded-[32px] shadow-2xl flex flex-col group overflow-hidden transition-all hover:bg-white/[0.01]">
                                ...
                                <div className="p-4 space-y-1 max-h-[300px] overflow-y-auto custom-scrollbar italic font-bold">
                                    {c.classUsers?.map((cu: any) => (
                                        <div key={cu.user.id} className="flex justify-between items-center group/item p-3 rounded-2xl hover:bg-white/[0.02] transition-all bg-black/20">
                                            <p className="font-black text-[9px] text-zinc-400 uppercase tracking-tighter truncate leading-none">{cu.user.name}</p>
                                            <button onClick={() => removeStudentFromClass(c.id, cu.user.id)} className="w-7 h-7 flex items-center justify-center rounded-lg text-red-500/10 hover:text-red-500 hover:bg-red-500/10 transition-all font-black text-[10px]">✕</button>
                                        </div>
                                    ))}
                                </div>
                          </div>
